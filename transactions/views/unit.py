...
from transactions.forms.unit import EntityUnitModelCreateForm, EntityUnitModelUpdateForm
from transactions.models import EntityUnitModel, EntityModel
from transactions.views.entity import FiscalYearEntityModelBalanceSheetView, FiscalYearEntityModelIncomeStatementView
from transactions.views.mixins import LoginRequiredMixIn, QuarterlyReportMixIn, MonthlyReportMixIn, DateReportMixIn


class EntityUnitModelListView(LoginRequiredMixIn, ListView):
    template_name = 'transactions/unit_list.html'
...
class EntityUnitModelDetailView(LoginRequiredMixIn, DetailView):
    template_name = 'transactions/unit_detail.html'
...
class EntityUnitModelCreateView(LoginRequiredMixIn, CreateView):
    template_name = 'transactions/unit_create.html'
    ...
    def get_success_url(self):
        return reverse('transactions:unit-list',
                       kwargs={
                           'entity_slug': self.kwargs['entity_slug']
                       })
...
class EntityUnitUpdateView(LoginRequiredMixIn, UpdateView):
    template_name = 'transactions/unit_update.html'
...
class EntityUnitModelBalanceSheetView(LoginRequiredMixIn, RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        year = localdate().year
        return reverse('transactions:unit-bs-year',
                       kwargs={
                           'entity_slug': self.kwargs['entity_slug'],
...
class EntityUnitModelIncomeStatementView(LoginRequiredMixIn, RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        year = localdate().year
        return reverse('transactions:unit-ic-year',
                       kwargs={
                           'entity_slug': self.kwargs['entity_slug'],
                           'unit_slug': self.kwargs['unit_slug'],
                           'year': year
