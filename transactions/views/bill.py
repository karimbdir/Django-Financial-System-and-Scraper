...
from transactions.forms.bill import BillModelCreateForm, BillModelUpdateForm, BillItemFormset, BillModelConfigureForm
from transactions.models import EntityModel, PurchaseOrderModel, LedgerModel
from transactions.models.bill import BillModel
from transactions.utils import new_bill_protocol, mark_accruable_paid
from transactions.views.mixins import LoginRequiredMixIn


class BillModelListView(LoginRequiredMixIn, ArchiveIndexView):
    template_name = 'transactions/bill_list.html'
...
class BillModelCreateView(LoginRequiredMixIn, CreateView):
    template_name = 'transactions/bill_create.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.for_purchase_order:
            ...
            form_action = reverse('transactions:bill-create-po',
                                  kwargs={
                                      'entity_slug': self.kwargs['entity_slug'],
                                      'po_pk': po_model.uuid
                                  }) + f'?item_uuids={po_item_uuids_qry_param}'
        else:
            form_action = reverse('transactions:bill-create',
                                  kwargs={
                                      'entity_slug': self.kwargs['entity_slug'],
                                  })
    ...
    def get_success_url(self):
        ...
        if self.for_purchase_order:
            po_pk = self.kwargs['po_pk']
            return reverse('transactions:po-update',
                           kwargs={
                               'entity_slug': entity_slug,
                               'po_pk': po_pk
                           })
        return reverse('transactions:bill-list',
                       kwargs={
                           'entity_slug': entity_slug
                       })
...
class BillModelUpdateView(LoginRequiredMixIn, UpdateView):
    ...
    template_name = 'transactions/bill_update.html'
    extra_context = {
        'header_subtitle_icon': 'uil:bill'
    }
    ...
    def get_success_url(self):
        ...
        return reverse('transactions:bill-detail',
                       kwargs={
                           'entity_slug': entity_slug,
                           'bill_pk': bill_pk
                       })
...
class BillModelItemsUpdateView(LoginRequiredMixIn, View):
    ...
    def post(self, request, entity_slug, bill_pk, **kwargs):
        ...
        return HttpResponseRedirect(reverse('transactions:bill-update',
                                            kwargs={
                                                'entity_slug': entity_slug,
                                                'bill_pk': bill_pk
                                            }))
...
class BillModelDetailView(LoginRequiredMixIn, DetailView):
    ...
    slug_field = 'uuid'
    context_object_name = 'bill'
    template_name = 'transactions/bill_detail.html'
    extra_context = {
        'header_subtitle_icon': 'uil:bill',
        'hide_menu': True
    }
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        if not bill_model.is_configured():
            link = format_html(f"""
            <a href="{reverse("transactions:bill-update", kwargs={
                'entity_slug': self.kwargs['entity_slug'],
                'bill_pk': bill_model.uuid
            })}">here</a>
            """)
...
class BillModelDeleteView(LoginRequiredMixIn, DeleteView):
    ...
    def get_template_names(self):
        if self.void:
            return 'transactions/bill_void.html'
        return 'transactions/bill_delete.html'
    ...
    def get_form_action_url(self):
        ...
        if self.void:
            return reverse(
                viewname='transactions:bill-void',
                kwargs=KWARGS
            )
        return reverse(
            viewname='transactions:bill-delete',
            kwargs=KWARGS
        )
        ...
        if self.void:
            ...
            if bill_model.void:
                messages.add_message(
                    ...
                    level=messages.ERROR,
                    extra_tags='is-danger')
                url_redirect = reverse('transactions:bill-detail',
                                       kwargs={
                                           'entity_slug': self.kwargs['entity_slug'],
                                           'bill_pk': bill_model.uuid
                                       })
...
class BillModelMarkPaidView(LoginRequiredMixIn,
    ...
    def post(self, request, *args, **kwargs):
        ...
        messages.add_message(request,
                             ...
                             extra_tags='is-success')
        redirect_url = reverse('transactions:bill-detail',
                               kwargs={
                                   'entity_slug': self.kwargs['entity_slug'],
                                   'bill_pk': bill.uuid
                               })
...
class BillModelActionView(LoginRequiredMixIn, SingleObjectMixin, RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        bill_model: BillModel = self.get_object()
        return reverse('transactions:bill-update',
                       kwargs={
                           'entity_slug': self.kwargs['entity_slug'],
                           'bill_pk': bill_model.uuid
                       })
