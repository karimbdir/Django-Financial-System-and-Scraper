...
from transactions.io import IOMixIn
from transactions.models.coa import ChartOfAccountModel
from transactions.models.mixins import CreateUpdateMixIn, SlugNameMixIn, ContactInfoMixIn, NodeTreeMixIn
...
class EntityModelAbstract(NodeTreeMixIn,
    ...
    def get_manage_url(self):
        return reverse('transactions:entity-update',
                       kwargs={
                           'entity_slug': self.slug
                       })

    def get_ledgers_url(self):
        return reverse('transactions:ledger-list',
                       kwargs={
                           'entity_slug': self.slug
                       })
    ...
    def get_balance_sheet_url(self):
        return reverse('transactions:entity-bs',
                       kwargs={
                           'entity_slug': self.slug
                       })

    def get_income_statement_url(self):
        return reverse('transactions:entity-ic',
                       kwargs={
                           'entity_slug': self.slug
                       })

    def get_data_import_url(self):
        return reverse('transactions:data-import-jobs-list',
                       kwargs={
                           'entity_slug': self.slug
                       })

    def get_accounts_url(self):
        return reverse('transactions:account-list',
                       kwargs={
                           'entity_slug': self.slug,
                       })

    def get_customers_url(self):
        return reverse('transactions:customer-list',
                       kwargs={
                           'entity_slug': self.slug,
                       })
    ...
    def get_delete_url(self):
        return reverse('transactions:entity-delete',
                       kwargs={
                           'entity_slug': self.slug
                       })
...
class EntityManagementModelAbstract(CreateUpdateMixIn):
    ...
    uuid = models.UUIDField(default=uuid4, editable=False, primary_key=True)
    entity = models.ForeignKey('transactions.EntityModel',
                               on_delete=models.CASCADE,
                               verbose_name=_('Entity'),
