...
from transactions.models.mixins import CreateUpdateMixIn, AccruableItemMixIn
...
class LazyLoader:
    ...
    def get_txs_model(self):
        if not self.TXS_MODEL:
            from transactions.models.transactions import TransactionModel
...
class InvoiceModelAbstract(AccruableItemMixIn, CreateUpdateMixIn):
    ...
    customer = models.ForeignKey('transactions.CustomerModel',
                                 on_delete=models.PROTECT,
                                 verbose_name=_('Customer'))

    cash_account = models.ForeignKey('transactions.AccountModel',
                                     on_delete=models.PROTECT,
                                     verbose_name=_('Cash Account'),
                                     related_name=f'{REL_NAME_PREFIX}_cash_account')
    prepaid_account = models.ForeignKey('transactions.AccountModel',
                                        on_delete=models.PROTECT,
                                        verbose_name=_('Prepaid Account'),
                                        related_name=f'{REL_NAME_PREFIX}_prepaid_account')
    unearned_account = models.ForeignKey('transactions.AccountModel',
                                         on_delete=models.PROTECT,
    ...
    invoice_items = models.ManyToManyField('transactions.ItemModel',
                                           through='transactions.ItemThroughModel',
    ...
    def get_mark_paid_url(self, entity_slug):
        return reverse('transactions:invoice-mark-paid',
                       kwargs={
