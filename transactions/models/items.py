...
from transactions.settings import transactions_CURRENCY_SYMBOL as currency_symbol
...
class ItemModelAbstract(CreateUpdateMixIn):
    ...
    uom = models.ForeignKey('transactions.UnitOfMeasureModel',
...
class ItemThroughModelAbstract(NodeTreeMixIn, CreateUpdateMixIn):
    ...
    uuid = models.UUIDField(default=uuid4, editable=False, primary_key=True)
    entity_unit = models.ForeignKey('transactions.EntityUnitModel',
                                    on_delete=models.SET_NULL,
                                    blank=True,
                                    null=True,
                                    verbose_name=_('Associated Entity Unit'))
    item_model = models.ForeignKey('transactions.ItemModel',
                                   on_delete=models.PROTECT,
                                   verbose_name=_('Item Model'))
    bill_model = models.ForeignKey('transactions.BillModel',
                                   on_delete=models.CASCADE,
                                   null=True,
                                   blank=True,
                                   verbose_name=_('Bill Model'))
    invoice_model = models.ForeignKey('transactions.InvoiceModel',
                                      on_delete=models.CASCADE,
                                      null=True, blank=True,
                                      verbose_name=_('Invoice Model'))
    po_model = models.ForeignKey('transactions.PurchaseOrderModel',
                                 on_delete=models.SET_NULL,
                                 null=True,
                                 blank=True,
